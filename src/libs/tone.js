...
// Shared synth for MIDI timeline playback - using PolySynth to allow multiple simultaneous notes
let timelineSynth = null

/**
 * Schedule MIDI notes based on relative timestamps
 * @param {Array<{relativeTime: number, midiNote: number, duration: number, velocity: number}>} notes - Array of notes to schedule
 * @returns {Array} Array of scheduled note references (for potential cancellation)
 */
export async function scheduleMidiNotes(notes) {
  ...
  // Ensure audio context is started (required by Tone.js)
  await Tone.start()

  // Create or reuse PolySynth for timeline (allows multiple simultaneous notes)
  if (!timelineSynth) {
    // Create a Synth with sinusoidal oscillator for smoother sound
    const synthOptions = {
      oscillator: {
        type: 'sine' // Sinusoidal wave for smooth, non-harsh sound
      },
      envelope: {
        attack: 0.1,
        decay: 0.2,
    ...
    // Use PolySynth to allow multiple notes to play simultaneously
  ...
  // Program each note with its relative time
  for (const note of notes) {
    const { relativeTime, midiNote, duration, velocity } = note
    
    // Calculate absolute time: baseTime + relativeTime
    const absoluteTime = baseTime + relativeTime
    
    // Schedule the note - PolySynth allows multiple notes to overlap
    timelineSynth.triggerAttackRelease(midiNote, duration, absoluteTime, velocity)
    
    // Store reference (though Tone.js doesn't provide direct cancellation, we keep track)
    scheduledNotes.push({
      midiNote,
...
/**
 * Cancel scheduled MIDI notes by releasing all notes from the timeline synth
 */
